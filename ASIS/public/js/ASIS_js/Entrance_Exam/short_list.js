...
function fetchTransactionsData(page, filters) {
    ...
    $.ajax({
        url: '/exam/short/listed-load?page=' + page,
        type: 'POST',
        data: filters,
        dataType: 'json',
        headers: {
        ...
        success: function (data) {
            ...
            }else
            {
                ...
                data.transactions.forEach(function (transaction) {
                    ...
                    if(transaction.remarks === 'PASSED')
                    {
                        action_button =
                            `<a class="flex items-center text-primary whitespace-nowrap btn_add_short_list" href="javascript:;"
                                    data-enrollees-id="${  transaction.enrollees_id  }"
                                    data-ratedlist-id="${  transaction.ratedList_id  }"
                    ...
                    {
                        action_button =
                            `<a class="flex items-center text-secondary whitespace-nowrap btn_result_cancelled" data-status="${ transaction.remarks }" href="javascript:;">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" icon-name="arrow-left-right" data-lucide="arrow-left-right" class="lucide lucide-arrow-left-right w-4 h-4 mr-1"><polyline points="17 11 21 7 17 3"></polyline><line x1="21" y1="7" x2="9" y2="7"></line><polyline points="7 21 3 17 7 13"></polyline><line x1="15" y1="17" x2="3" y2="17"></line></svg>
                                        Add to Short List
                            </a>`;
                    }
                        ...
                        <td class="text-center">
                            <div class="flex justify-center items-center">
                                <a href="javascript:;" class="box flex items-center px-3 py-2 rounded-md bg-white/10 dark:bg-darkmode-700 font-medium">
                                    <div class="w-2 h-2 bg-${ transaction.remarks_class } rounded-full mr-3"></div> <span class="text-${ transaction.remarks_class }">${ transaction.exam_result }</span>
                                </a>
                            </div>
                        </td>
                        <td class="text-center">
                            <div class="flex justify-center items-center">
                                <a href="javascript:;" class="box flex items-center px-3 py-2 rounded-md bg-white/10 dark:bg-darkmode-700 font-medium">
                                    <div class="w-2 h-2 bg-${ transaction.remarks_class } rounded-full mr-3"></div> <span class="text-${ transaction.remarks_class }">${ transaction.stanine }</span>
                                </a>
                            </div>
                        </td>
                        <td class="text-center">
                            <div class="flex justify-center items-center">
                                <a href="javascript:;" class="box flex items-center px-3 py-2 rounded-md bg-white/10 dark:bg-darkmode-700 font-medium">
                                    <div class="w-2 h-2 bg-${ transaction.remarks_class } rounded-full mr-3"></div> <span class="text-${ transaction.remarks_class }">${ transaction.remarks }</span>
                                </a>
                            </div>
                        </td>
                        <td class="table-report__action">
...
function examineesListActions(){
    ...
    $('body').on('click', '.btn_add_short_list', function(){

        __modal_toggle('rated_list_mdl');

        let enrollees_id = $(this).data('enrollees-id');
        let ratedlist_id = $(this).data('ratedlist-id');
        //
        $('.input_exam_list_id').val(ratedlist_id);
        // $('.stanine_score').val(examinee_stanine);

    });


    $('body').on('click', '.btn_add_to_shortList', function(){

        let this_button = $(this);

        let form_data = {
            _token,
            ratedlist_id     :   $('.input_exam_list_id').val(),


        }
        $.ajax({
            url: bpath + 'exam/rated/update/status',
            type: "POST",
            data: form_data,
            beforeSend: function () {
