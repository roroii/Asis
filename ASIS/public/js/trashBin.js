...
$(document).ready(function (){
    ...
    load_trashBinDataTable();
    load_trashDocs();

});

function load_trashBinDataTable(){

    try{
        /***/
        tbl_data_trashDocs = $('#dt__trashDocs').DataTable({
            dom:
...
function load_trashDocs() {

    $.ajax({
        url: bpath + 'documents/trashBin/docs-trash',
        type: "POST",
        data: {
            _token: _token,
        },
            ...
            if(data.length > 0) {
                for(var i=0;i<data.length;i++) {
                    ...
                    cd = '' +
                        ...
                        '<a id="btn_viewAttachments" href="javascript:;" data-trk-no="'+track_number+'"> <div class="flex justify-center  whitespace-nowrap "><i class="fa fa-folder w-4 h-4 mr-2 text-'+status_class+'"></i>'+type_submitted+'</div></a>' +
                        '</td>' +
                    ...
                    tbl_data_trashDocs.row.add($(cd)).draw();
                    /***/
                }
