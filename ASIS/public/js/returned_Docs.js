$(document).ready(function (){
    ...
    load_returnedDocsDataTable();
    load_returnedDocs();

});
var  _token = $('meta[name="csrf-token"]').attr('content');
var tbl_data_returnedDocs;

function load_returnedDocsDataTable(){

    try{
        /***/
        tbl_data_returnedDocs = $('#dt__returnedDocs').DataTable({
            dom:
...
function load_returnedDocs() {

    $.ajax({
        url: bpath + 'documents/returned/returned-docs/load',
        type: "POST",
        data: {
            _token: _token,
        },
        success: function(data) {
            ...
            tbl_data_returnedDocs.clear().draw();
            /***/
                ...
                for(var i=0;i<data.length;i++) {
                    ...
                    cd = '' +
                        ...
                        '<td class="status">'+
                            ...
                            '<div class="flex items-center whitespace-nowrap text-'+status_class+'"><div class="w-2 h-2 bg-'+status_class+' rounded-full mr-3"></div>Returned</div>' +
                        ...
                        tbl_data_returnedDocs.row.add($(cd)).draw();
                    /***/
                }
